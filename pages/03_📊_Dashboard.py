...
Clean_data = pd.read_csv("Datasets/Cleaned_data")
...
senior_citizen = Clean_data[Clean_data['SeniorCitizen'] == 'Yes']
...
payment_churn = Clean_data.groupby('PaymentMethod')['Churn'].value_counts(normalize=True).unstack()
churn_rate = payment_churn['Yes']

# Churn rate by Internet Service
internet_churn_group = Clean_data.groupby('InternetService')['Churn'].value_counts(normalize=True)
...
contract_churn_rate = Clean_data.groupby('Contract')['Churn'].value_counts().unstack()
...
numerical_data = Clean_data[numerical_columns]
...
elif visualization_category == "Univariate Analysis":
    with col1:
        fig5 = px.histogram(Clean_data, x='TotalCharges', nbins=20, title="Histogram of Tenure")
        st.plotly_chart(fig5, use_container_width=True)

    with col2:
        fig6 = px.histogram(Clean_data, x='MonthlyCharges', nbins=20, title="Histogram of Monthly Charges")
        st.plotly_chart(fig6, use_container_width=True)
        
        fig7 = px.histogram(Clean_data, x='Tenure', nbins=20, title="Histogram of Tenure")
